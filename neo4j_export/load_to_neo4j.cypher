...
// Reemplaza <BASE_URL> por tu URL raw de GitHub, ej:
...
// ---- NODOS ----
LOAD CSV WITH HEADERS FROM '<BASE_URL>/nodes/tournaments.csv' AS row
...
LOAD CSV WITH HEADERS FROM '<BASE_URL>/nodes/matches.csv' AS row
...
LOAD CSV WITH HEADERS FROM '<BASE_URL>/nodes/awards.csv' AS row
...
// ---- RELACIONES ----
LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/edition_part_of_tournament.csv' AS row
MATCH (a:Edition {id: row.`:START_ID`}), (b:Tournament {id: row.`:END_ID`})
MERGE (a)-[:PART_OF]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/match_in_edition.csv' AS row
MATCH (a:Match {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:IN_EDITION]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/match_at_stage.csv' AS row
MATCH (a:Match {id: row.`:START_ID`}), (b:Stage {id: row.`:END_ID`})
MERGE (a)-[:AT_STAGE]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/team_played_match.csv' AS row
...
LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/team_participated_in_edition.csv' AS row
MATCH (a:Team {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:PARTICIPATED_IN]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/team_reached_stage.csv' AS row
MATCH (a:Team {id: row.`:START_ID`}), (b:Stage {id: row.`:END_ID`})
MERGE (a)-[r:REACHED_STAGE]->(b)
SET r.stageOrder = toInteger(row.STAGE_ORDER);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/team_used_name.csv' AS row
MATCH (a:Team {id: row.`:START_ID`}), (b:TeamName {id: row.`:END_ID`})
MERGE (a)-[:USED_NAME]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/player_scored_in_edition.csv' AS row
...
LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/player_carded_in_match.csv' AS row
...
LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/player_played_for_team.csv' AS row
MATCH (a:Player {id: row.`:START_ID`}), (b:Team {id: row.`:END_ID`})
MERGE (a)-[r:PLAYED_FOR]->(b)
SET r.editionId = row.EDITION_ID;

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/award_for_team.csv' AS row
MATCH (a:Award {id: row.`:START_ID`}), (b:Team {id: row.`:END_ID`})
MERGE (a)-[:FOR_TEAM]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/award_in_edition.csv' AS row
MATCH (a:Award {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:IN_EDITION]->(b);

LOAD CSV WITH HEADERS FROM '<BASE_URL>/relationships/player_won_award.csv' AS row
