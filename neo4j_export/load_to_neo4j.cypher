...
// ---- NODOS ----
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/tournaments.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/editions.csv' AS row
MERGE (n:Edition {id: row.EDITION_ID})
SET n.year = row.YEAR, n.name = row.NAME, n.format = row.FORMAT, n.status = row.STATUS;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/stages.csv' AS row
MERGE (n:Stage {id: row.STAGE_ID})
SET n.name = row.NAME, n.order = toInteger(row.ORDER);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/teams.csv' AS row
MERGE (n:Team {id: row.TEAM_ID})
SET n.name = row.CANONICAL_NAME, n.active2026 = row.ACTIVE_2026, n.notes = row.NOTES;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/team_names.csv' AS row
MERGE (n:TeamName {id: row.NAME_ID})
SET n.name = row.NAME, n.nameType = row.NAME_TYPE;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/players.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/matches.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/nodes/awards.csv' AS row
...
// ---- RELACIONES ----
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/edition_part_of_tournament.csv' AS row
MATCH (a:Edition {id: row.`:START_ID`}), (b:Tournament {id: row.`:END_ID`})
MERGE (a)-[:PART_OF]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/match_in_edition.csv' AS row
MATCH (a:Match {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:IN_EDITION]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/match_at_stage.csv' AS row
MATCH (a:Match {id: row.`:START_ID`}), (b:Stage {id: row.`:END_ID`})
MERGE (a)-[:AT_STAGE]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/team_played_match.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/team_participated_in_edition.csv' AS row
MATCH (a:Team {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:PARTICIPATED_IN]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/team_reached_stage.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/team_used_name.csv' AS row
MATCH (a:Team {id: row.`:START_ID`}), (b:TeamName {id: row.`:END_ID`})
MERGE (a)-[:USED_NAME]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/player_scored_in_edition.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/player_carded_in_match.csv' AS row
...
LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/player_played_for_team.csv' AS row
MATCH (a:Player {id: row.`:START_ID`}), (b:Team {id: row.`:END_ID`})
MERGE (a)-[r:PLAYED_FOR]->(b)
SET r.editionId = row.EDITION_ID;

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/award_for_team.csv' AS row
MATCH (a:Award {id: row.`:START_ID`}), (b:Team {id: row.`:END_ID`})
MERGE (a)-[:FOR_TEAM]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/award_in_edition.csv' AS row
MATCH (a:Award {id: row.`:START_ID`}), (b:Edition {id: row.`:END_ID`})
MERGE (a)-[:IN_EDITION]->(b);

LOAD CSV WITH HEADERS FROM 'https://raw.githubusercontent.com/jsaraviadrago/verbose-system/main/Grafos/neo4j_export/relationships/player_won_award.csv' AS row
